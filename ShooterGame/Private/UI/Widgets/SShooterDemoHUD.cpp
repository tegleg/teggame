...
int32 SShooterReplayTimeline::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyClippingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	if ((ImageBrush != nullptr) && (ImageBrush->DrawAs != ESlateBrushDrawType::NoDrawType) && DemoDriver.IsValid())
	{
		...
		const FColor FinalColorAndOpacity( InWidgetStyle.GetColorAndOpacityTint() * ColorAndOpacity.Get() * ImageBrush->GetTint( InWidgetStyle ) );
		...
		FSlateRect NewClippingRect;
		NewClippingRect.Left = MyClippingRect.Left + Center.X - (ImageBrush->ImageSize.X * 0.5f);
		NewClippingRect.Right = MyClippingRect.Left + Center.X + (ImageBrush->ImageSize.X * 0.5f);
		NewClippingRect.Bottom = MyClippingRect.Bottom - Center.Y + (ImageBrush->ImageSize.Y * 0.5f);
		NewClippingRect.Top = MyClippingRect.Top + Center.Y - (ImageBrush->ImageSize.Y * 0.5f);

		const int32 IndicatorLayerId = ParentLayerId + 1;
			
		FSlateDrawElement::MakeBox(OutDrawElements, IndicatorLayerId, AllottedGeometry.ToPaintGeometry(), ImageBrush, NewClippingRect, DrawEffects, FinalColorAndOpacity );
