...
void SShooterDemoList::BuildDemoList()
{
	...
	{
		ReplayStreamer->EnumerateStreams(EnumerateStreamsVersion, FOnEnumerateStreamsComplete::CreateSP(this, &SShooterDemoList::OnEnumerateStreamsComplete));
	}
}
		...
		if ( GI != NULL )
		{
			...
			if ( ShooterViewport )
			{
				ShooterViewport->ShowDialog( 
					...
					FText::Format(LOCTEXT("DeleteDemoFmt", "Delete {0}?"), FText::FromString(SelectedItem->StreamInfo.Name)),
