...
const FName ACar::LookUpBinding("LookUp");
const FName ACar::LookRightBinding("LookRight");
const FName ACar::EngineAudioRPM("RPM");

#define LOCTEXT_NAMESPACE "VehiclePawn"

ACar::ACar(const class FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	static ConstructorHelpers::FClassFinder<UObject> AnimBPClass(TEXT("/Game/Sedan/Sedan_AnimBP.Sedan_AnimBP"));
	GetMesh()->SetAnimationMode(EAnimationMode::AnimationBlueprint);
	GetMesh()->SetAnimInstanceClass(AnimBPClass.Class);

	// Setup friction materials
	static ConstructorHelpers::FObjectFinder<UPhysicalMaterial> SlipperyMat(TEXT("/Game/Environment/PhysicalMaterials/M_Concrete.M_Concrete"));
	SlipperyMaterial = SlipperyMat.Object;

	static ConstructorHelpers::FObjectFinder<UPhysicalMaterial> NonSlipperyMat(TEXT("/Game/Environment/PhysicalMaterials/M_Concrete.M_Concrete"));
	...
	// Wheels/Tyres
	// Setup the wheels
	Vehicle4W->WheelSetups[0].WheelClass = UCarWheel::StaticClass();
	Vehicle4W->WheelSetups[0].BoneName = FName("PhysWheel_FL");
	Vehicle4W->WheelSetups[0].AdditionalOffset = FVector(0.f, -8.f, 0.f);

	Vehicle4W->WheelSetups[1].WheelClass = UCarWheel::StaticClass();
	Vehicle4W->WheelSetups[1].BoneName = FName("PhysWheel_FR");
	Vehicle4W->WheelSetups[1].AdditionalOffset = FVector(0.f, 8.f, 0.f);

	Vehicle4W->WheelSetups[2].WheelClass = UCarWheel::StaticClass();
	Vehicle4W->WheelSetups[2].BoneName = FName("PhysWheel_BL");
	Vehicle4W->WheelSetups[2].AdditionalOffset = FVector(0.f, -8.f, 0.f);

	Vehicle4W->WheelSetups[3].WheelClass = UCarWheel::StaticClass();
	...
	static ConstructorHelpers::FObjectFinder<USoundCue> SoundCue(TEXT("SoundCue'/Game/Sounds/AmbientLoops/Stereo/RoomTone_AbstractAnimated03_Stereo_Cue.RoomTone_AbstractAnimated03_Stereo_Cue'"));
...
void ACar::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
	...
	InputComponent->BindAction("Handbrake", IE_Pressed, this, &ACar::OnHandbrakePressed);
	InputComponent->BindAction("Handbrake", IE_Released, this, &ACar::OnHandbrakeReleased);
	InputComponent->BindAction("SwitchCamera", IE_Pressed, this, &ACar::OnToggleCamera);

	InputComponent->BindAction("ResetVR", IE_Pressed, this, &ACar::OnResetVR);
}
...
void ACar::OnToggleCamera()
{
	EnableIncarView(!bInCarCameraActive);
}

void ACar::EnableIncarView(const bool bState)
{
...
void ACar::UpdatePhysicsMaterial()
{
